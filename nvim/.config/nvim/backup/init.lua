...
local function setup_obsidian()
  -- Change this to the actual path of your notes directory.
  -- Using vim.fn.expand() ensures ~ is resolved correctly.
  local notes_path = vim.fn.expand("~/code-playground/obsidian/notes")

  -- Only configure obsidian.nvim if the workspace directory exists.
  -- This prevents the "At least one workspace is required!" error.
  if vim.fn.isdirectory(notes_path) == 0 then
    vim.notify(
      "obsidian.nvim disabled: workspace does not exist: " .. notes_path,
      vim.log.levels.WARN
    )
    return
  end
